...
export default function EditBidModal({ isVisible, onClose }: EditBidModalProps) {
  ...
  return (
    <SlideModal isVisible={isVisible} onClose={onClose}>
      <View style={s.headerRow}>
        <Text style={s.title}>Edit bid</Text>
      ...
      <View style={s.chipsRow}>
        {[120, 110, 100, 90].map((amount) => (
          <TouchableOpacity
            ...
            style={[
              s.chip,
              { backgroundColor: selectedBid === amount ? '#000' : '#E5E7EB' },
            ]}>
            <Text
              style={[
                s.chipText,
                { color: selectedBid === amount ? '#fff' : '#000' },
              ]}>
              {amount}
            </Text>
          </TouchableOpacity>
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          onClose();
          Alert.alert('Bid Updated!', `Your bid has been updated to $${selectedBid}.00.`, [{ text: 'OK' }]);
        }}>
        <Text style={s.placeBtnText}>Update bid</Text>
      </TouchableOpacity>
    </SlideModal>
  );
}
